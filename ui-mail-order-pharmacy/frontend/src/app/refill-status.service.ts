...
export class RefillStatusService {
  ...
  baseUrl: string = 'http://localhost:8048/refill-service/refillappdb'; //http://localhost:8048/refill-service/refillappdb
  ...
  getAllRefillStatus(id: number): Observable<RefillStatus[]> {
    ...
    return this.http.get<RefillStatus[]>(
      `${this.baseUrl}/viewRefillStatus/${localStorage.getItem('subID')}`,
      header
    );
  }
  ...
  getRefillDue(date: number) {
    ...
    return this.http.get<RefillDue[]>(
      `${this.baseUrl}/getRefillDuesAsOfDate/${localStorage.getItem(
        'userID'
      )}/${date}`,
      header
    );
